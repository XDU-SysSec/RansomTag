...
#read the DATA Attribution of MFT record
def read_runlist_mft(f, bpc, runlist):
    out = bytearray()
    runlists=[]
    versions=10
    
    for rlen, roff in runlist:
        clusterlist = [[[] for x in range(versions) ] for y in range(rlen)]
        cluster=roff
        print(rlen, roff)
        for i in range(rlen):
            for j in range(versions):
                clusterlist[i][j]=readat(f, cluster * bpc, 1 * bpc)
            cluster+=1
        runlists.append((clusterlist, rlen))
    

    for c in range(versions):
        for clusterlist,rlen in runlists:
            for b in range(rlen):
                out+=clusterlist[b][c]
...
'''
        for i in range(rlen):
            cluster_cur = bytearray()
            for j in range(10):
                cluster_cur += readat(f, cluster * bpc, 1 * bpc)
            if (cluster_cur[:5].decode('utf8') == 'FILE0'):
                #print(cluster_pre[:5])
                out += cluster_pre
                print(cluster_pre[:5])
                cluster_pre = cluster_cur
                cluster_cur=cluster_cur.clear()
                print("if cluster_pre",cluster_pre[:5])
            else:
                x=0
                for k in range(10):
                    out+=cluster_pre[x:x+bpc]
                    print("else:",cluster_pre[:5])
                    out+=cluster_cur[x:x+bpc]
                    print("else2:",cluster_cur[:5])
                    x+=bpc
                cluster_pre=cluster_cur
'''           
...
def parse_attr(f, bpc, chunk):
    ...
    if namelen:
        name = chunk[nameoff:nameoff+namelen*2].decode('UTF-16-LE')   #属性名
    else:
        name = None
...
#parse MFT itself
def parse_attr_mft(f, bpc, chunk):
    type, size, nonres, namelen, nameoff = struct.unpack('<iiBBH', chunk[:12])
    ...
    if nonres:
        ...
        while rlpos < len(chunk):
            ...
            runlist.append((thislen, curoff))
        if sname == 'DATA':
            attrdata = lambda: sparser(read_runlist_mft(f, bpc, runlist))
        else:
            attrdata = lambda: sparser(read_runlist(f, bpc, runlist)[:size_actual])
    else:
        attrlen, attroff = struct.unpack('<IH', chunk[16:22])
...
#Find the DATA Attribution of MFT 
def parse_file_mft(f, chunkoff, bpc, chunk):
    magic, usa_ofs, usa_count, lsn, seq, link, attr_offset = struct.unpack(
        '<IHHQHHH', chunk[:22])
    attrs = collections.defaultdict(dict)
    try:
    ...
    while 1:
        ...
        try:
            sname, name, data = parse_attr_mft(f, bpc, chunk[pos:pos+size])
            attrs[sname][name] = data
        except Exception as e:
            print("File at offset %d: failed to parse attr type=%d pos=%d: %s" % (chunkoff, type, pos, e), file=sys.stderr)
...
def read_mft(f, bpc, mft_cluster, clusters_per_mft):
    ...
    #try:
    mftattr = parse_file_mft(f, 0, bpc, mft[:1024])  
    newmft = mftattr['DATA'][None]()
    print("mft len:",len(newmft))  ##
    return newmft
    '''
