...
static PyObject *
method_pread (PyObject * self, PyObject * args)
{
  int fd;
  void *alignedbuff = NULL;
  PyObject *pyresult;
  ...
  if (!PyArg_ParseTuple (args, "ill", &fd, &count, &offset))
    return NULL;
  ...
  ret = pread (fd, alignedbuff, count, offset);
  Py_END_ALLOW_THREADS;
...
ignoring the buffer cache.\n"},
  {"pread", method_pread, METH_VARARGS,
